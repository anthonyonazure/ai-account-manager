...
def briefing_has_actions_or_states_quiet(state: Mapping[str, Any]) -> tuple[bool, str]:
    actions = state.get("actions") or []
    md = (state.get("markdown") or "").lower()
    if actions:
        return True, f"{len(actions)} actions present"
    if (
        "no priority actions" in md
        or "nothing to action" in md
        or "all your accounts" in md
    ):
        return True, "explicitly states quiet day"
    return False, "no actions and no quiet-day acknowledgment"


def top_action_matches_highest_weighted_score(
...
def narrative_mentions_top_account(state: Mapping[str, Any]) -> tuple[bool, str]:
    actions = state.get("actions") or []
    ...
    return (
        top_name in narrative,
        f"top account {top_name!r} {'in' if top_name in narrative else 'NOT in'} markdown",
    )


def at_least_one_specific_number(state: Mapping[str, Any]) -> tuple[bool, str]:
...
def no_banned_tropes(state: Mapping[str, Any]) -> tuple[bool, str]:
    md = (state.get("markdown") or "").lower()
    hits = [t for t in BANNED_TROPES if t in md]
    return len(hits) == 0, f"tropes found: {hits}" if hits else "no tropes"


def narrative_length_reasonable(state: Mapping[str, Any]) -> tuple[bool, str]:
    md = state.get("markdown") or ""
    ...
    narrative = next(
        (b for b in blocks if not b.startswith("##") and not b.startswith("_")), ""
    )
...
def evaluate_briefing(state: Mapping[str, Any], am_email: str) -> dict:
    results = []
